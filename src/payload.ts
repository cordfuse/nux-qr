/**
 * payload.ts — the NUX MightyAmp PAYLOAD encoder. Pure logic, no I/O.
 *
 * Split out of qr-generator.ts so it can be bundled for a BROWSER.
 *
 * Why this file exists: the CLI bundle is ~1 MB, because it carries the image
 * renderer (pureimage), opentype, and two base64 TTF fonts. None of that is needed
 * to turn a preset into a `nux://MightyAmp:...` string — that is a couple of hundred
 * lines of arithmetic. A ToneAI agent running in a browser (Claude web) has no Node
 * and no shell, so it renders the QR client-side; it needs the ENCODER and nothing
 * else, and it cannot carry a megabyte to get it.
 *
 * NOTHING HERE MAY IMPORT NODE. No fs, no zlib, no streams, no fonts, no pureimage.
 * If an import creeps in, the browser bundle silently balloons or breaks — there is a
 * test that fails the build if this file's bundle exceeds its size budget.
 *
 * The CLI imports from here, so there is exactly ONE implementation of the payload
 * format. A second one would be a second set of bugs, and a wrong payload is a wrong
 * tone that looks completely fine until it reaches the amp.
 */
...
/** Pro-family payloads carry the preset name in the bytes; the others don't. */
export const isProDevice = (d: DeviceType): boolean => PRO_DEVICES.has(d)
...
// ── Bytes ─────────────────────────────────────────────────────────────────────
//
// Uint8Array, not Buffer. Buffer is a Node type, and this module has to run in a
// browser too (Claude web renders the QR client-side). These four helpers are the
// entire surface the encoder used, so depending on Node for them was a needless
// portability tax — and a shim in the browser bundle would be a second place for the
// byte layout to go subtly wrong.

const alloc = (n: number, fill = 0): Uint8Array => new Uint8Array(n).fill(fill)
const bytes = (v: number[] | Uint8Array): Uint8Array => Uint8Array.from(v)

/** Node's 'ascii' encoding clears the high bit; match it byte for byte. */
const ascii = (s: string): Uint8Array =>
  Uint8Array.from(Array.from(s, (c) => c.charCodeAt(0) & 0x7f))

const concat = (list: Uint8Array[]): Uint8Array => {
...
/** Standard base64. Node has Buffer, browsers have btoa; neither is universal, so
 *  encode it here and be certain the two produce the same string. */
const B64 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
function base64(b: Uint8Array): string {
  let out = ''
  for (let i = 0; i < b.length; i += 3) {
    const a0 = b[i]!, a1 = b[i + 1], a2 = b[i + 2]
    const n = (a0 << 16) | ((a1 ?? 0) << 8) | (a2 ?? 0)
    out += B64[(n >> 18) & 63]! + B64[(n >> 12) & 63]!
    out += a1 === undefined ? '=' : B64[(n >> 6) & 63]!
    out += a2 === undefined ? '=' : B64[n & 63]!
  }
  return out
}
...
function buildProPayload(p: PresetParams): Uint8Array {
  const data = alloc(113, 0)
  ...
  data.set(ascii(qrName), 98)
  return data
}

function buildPlugAirPayload(p: PresetParams): Uint8Array {
  const data = alloc(40, 0)
...
function buildLitePayload(p: PresetParams): Uint8Array {
  const data = alloc(40, 0)
  data[0] = stdEnable(p.noise_gate.enabled); data[1] = p.noise_gate.sensitivity; data[2] = p.noise_gate.decay
...
function build8BTPayload(p: PresetParams): Uint8Array {
  const data = alloc(40, 0)
  data[0] = stdEnable(p.noise_gate.enabled); data[1] = p.noise_gate.sensitivity; data[2] = p.noise_gate.decay
  data[7] = p.amp.gain; data[8] = p.amp.master; data[12] = p.amp.param6 ?? 50
  data[13] = stdEnable(p.modulation?.enabled ?? false); data[14] = p.modulation?.id ?? 0; data[15] = p.modulation?.p1 ?? 0; data[16] = p.modulation?.p2 ?? 0
...
function build2040BTPayload(p: PresetParams): Uint8Array {
  const data = alloc(40, 0)
...
export function buildQRString(p: PresetParams): string {
  ...
  const full = concat([bytes([device.deviceQRId, device.deviceQRVersion]), payload])
  return 'nux://MightyAmp:' + base64(full)
}
